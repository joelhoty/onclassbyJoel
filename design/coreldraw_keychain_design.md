...
> **最後更新時間**: 2026-09-15 09:33:54 (UTC+8)  
> **使用模型**: Gemini 3.6 Flash  
> **執行 Agent**: CorelDRAW Design Specialist  
...
4. **貝茲曲線擬合與平滑化 (Bézier Curve Fitting & Optimization)**：
   * 利用**最小二乘法 (Least Squares Fitting)** 與 Potrace 演算法，將離散的像素路徑轉換為包含起始點、終點與控制點的**三次方貝茲曲線 (Cubic Bézier Curves)**。
...
| **檔案量成長曲線** | 隨著圖像實體尺寸與 DPI 增加呈現幾何級數暴增 | 僅取決於圖形邊界與節點 (Node) 數量，尺寸放大檔案大小不變 |
...
* **建立界線 (Create Boundary / 舊稱或口語常稱為「建議界線」)**：
...
#### 步驟 3：解散群組與清理
1. 選取描繪好的向量圖形，按下 `Ctrl + U` (解散群組) 或 `Ctrl + All Ungroup` (全部解散群組)。
...
* **解法**：使用 `形狀工具` 將開口節點拖曳重疊，並點擊 `連接兩個節點 (Join Two Nodes)` 或使用 `智慧型填款工具 (Smart Fill Tool)` 在封閉區域點擊重新生成單一封閉物件。
