...
在極近危險區 ($d \le 5\text{ cm}$)，若希望聲音隨距離縮短而**依反比 (1/d) 關係急遽升高**，採用雙曲倒數公式：
...
1. **頻率上下限截斷 (Pitch Clamping)**：EV3 Sound 積木的單音範圍為 $250 \sim 10000\text{ Hz}$，但主機喇叭小、高頻聽感刺耳，建議以截斷函數把音頻限制在 $250\text{ Hz} \sim 2500\text{ Hz}$ 的舒適範圍。
2. **音效持續時間優化**：將原本的 `1.0 秒` 播放時間調縮為 `0.1 ~ 0.2 秒`，才能實現連續滑順的調頻與高響應度倒車雷達體驗。
3. **感測器盲區**：EV3 超音波感測器在約 3 cm 以內讀值不可靠（有效範圍約 3~250 cm），$d = 1 \sim 2\text{ cm}$ 的頻率計算僅為理論示意。
...
- **變更摘要**: 全面檢視概念與敘述正確性並修正：
  - 1/d 為反比關係（非幾何級數）；EV3 單音範圍 250~10000 Hz，限制 2500 Hz 是舒適度考量而非失真
  - 補充超音波感測器約 3 cm 以內為盲區
