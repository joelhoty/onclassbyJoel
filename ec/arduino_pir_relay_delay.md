...
3. **離去精確延遲關燈（Off Delay 5 Seconds）**：  
   當人離開感應範圍後，PIR 訊號轉為低電位（Pin 4 = `LOW`）。系統啟動 **5 秒（5000ms）** 倒數計時。若 5 秒內無人再次經過，確認倒數逾時，Pin 2 轉為 `LOW`，繼電器釋放斷開，電燈熄滅。
...
* 採用 **`millis()` 時間戳記比對**：  
  CPU 主迴圈以每秒數萬次的速度極速輪詢，既能在人離開後精準執行 5 秒倒數，又能隨時在 1 毫秒內捕捉到人員重新進入的事件！
...
2. **並聯續流二極體（1N4007）**：線圈斷開時會產生極高反向感應電動勢（Back-EMF），必須在線圈兩端**反向並聯 1N4007 續流二極體**以消除高壓電弧脈衝。
3. **或直接選用商用 5V 繼電器模組**：市售 Arduino 繼電器模組板載已完整內建 **光耦合器 (PC817)**、**驅動電晶體** 與 **續流二極體**，可直接以 Pin 2 安全推動。
