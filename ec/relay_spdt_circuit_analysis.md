...
依據**安培環路定律**（線圈建立磁場）與**磁路能量原理**（產生吸力），當電流 $I$ 通入線圈時：
...
2. **雜訊阻絕**：高功率負載（如感性馬達、電磁閥開關）產生的電氣突波、大電流突入與地迴路雜訊（Ground Loop Noise）完全不會傳遞回敏感的微控制器核心。
...
LED 為非線性電流驅動元件，其導通壓降幾乎恆定，若未串聯限流電阻直接接入 6V 電源，將導致電流過大而瞬間過流燒毀。
...
| **切換速度** | 較慢 ($5\text{ ms} \sim 15\text{ ms}$) | **較快**（DC MOSFET 型為 $\mu\text{s}$ 級；AC 零交越型最長約半週期 8~10 ms） |
...
- **變更摘要**: 全面檢視概念與敘述正確性並修正：
  - LED 無限流電阻屬過流燒毀（非 Thermal Runaway）；Ground Bounce 改為地迴路雜訊（Ground Loop）
  - SSR 切換速度區分 DC MOSFET 型與 AC 零交越型；吸力原理改引安培環路定律與磁路能量
