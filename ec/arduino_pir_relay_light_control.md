...
人體表面輻射出的熱紅外線能量峰值落在長波紅外（LWIR，又稱熱紅外）區間（約 $8 \sim 14\,\mu\text{m}$）。

#### (1) 熱釋電晶體與雙元件差動結構
* PIR 感測器內部封裝了一組具有**熱釋電效應（Pyroelectric Effect）**的晶體（如鈦酸鉛鋯 PZT 或硫酸三甘氨酸 TGS）。當溫度發生微弱動態變化時，晶體表面電荷平衡被打破，產生極微弱的電荷訊號（電流約為皮安 pA 等級，需由感測頭內建的 JFET 放大）。
* 為避免環境氣溫起伏、太陽光散射造成的誤報，PIR 感測頭採用**雙差動元件（Dual-element Differential）**設計。當環境整體均勻受熱時，兩元件感應電壓相互抵消；唯有當移動目標依序穿過兩元件的視場時，才會產生一正一負的交變電壓脈衝。

#### (2) 菲涅耳透鏡 (Fresnel Lens) 聚光與分區
感測頭外層的白色半球形塑膠外殼即為菲涅耳透鏡。它透過等距同心圓刻痕，在超薄結構下將周邊空間分割成數十個交替排列的**敏感區（Active Zones）**與**盲區（Blind Zones）**。人體移動時連續切換明暗感應帶，大幅增強差動檢測靈敏度，有效感應距離約 3~7 公尺（可調），感應錐角約 110° 以內。
...
微控制器（MCU）如 ATmega328P 的 I/O 腳位輸出電壓僅為 5V，最大安全推動電流約為 20mA，無法直接驅動需要數十至上百毫安培電流（常見 5V 繼電器約 70~90 mA）的電磁線圈，更嚴禁直接接觸 AC 110V/220V 市電。

#### (1) 光耦隔離 (Optocoupler Isolation)
部分繼電器模組板載 PC817 光電耦合器，Arduino 輸出訊號僅負責點亮光耦內部的紅外發光二極體，藉由光訊號觸發次級側的光敏三極管。需特別注意：**弱電與強電 AC 110V 之間的隔離主要由繼電器本身的線圈／接點絕緣提供**；光耦只有在模組移除 JD-VCC 跳線、線圈側改用獨立電源時，才能讓 Arduino 與線圈驅動電路達到真正的電氣隔離（Galvanic Isolation）；若兩者共用 VCC／GND，則並未隔離。
...
| :--- | :--- | :--- |
| **CPU 狀態** | 處於忙等待（Busy Wait），時鐘空轉 5000ms | 主迴圈不被卡住（本例含 `delay(10)`，約每 10 ms 輪詢一次） |
| **動態響應** | 延遲期間主程式**無法輪詢任何輸入**（僅硬體中斷服務程式仍會執行），無法及時重置 | 隨時都在讀取 Pin 7，人再度進來可立刻刷新計時 |
| **多工擴充性** | 主迴圈無法同時處理串口指令、LED 閃爍、按鈕輪詢 | 可輕鬆擴充數十個並行任務（感測、通訊、顯示） |
...
| :--- | :--- | :--- | :--- |
| **PIR 感測器 (HC-SR501)** | VCC | 5V | 工作電源正極 (模組規格約 4.5V~20V) |
| | GND | GND | 電源接地共用端 |
...
void loop() {
    ...
    // 短暫微休 10ms，降低輪詢頻率 (PIR 輸出已是穩定數位訊號，並非嚴格意義的消抖)
    delay(10);
}
```
...
2. **軟體雙條件判斷**：亦可在 Arduino 類比腳位（如 A0）外接光敏電阻模組：
   ```cpp
   int ambientLight = analogRead(A0); // 依分壓接法而定：此處假設數值越小代表環境越暗
...
繼電器接點在切斷交流電感性或容性照明負載（如傳統日光燈鎮流器、高功率 LED 驅動器）瞬間，接點間隙會產生強烈的**高壓電弧（Spark/Arcing）**，造成接點燒蝕氧化；至於電容性負載（如 LED 驅動器）閉合瞬間的湧浪電流，則是接點熔焊黏死（Contact Welding）的主因。
* **RC 吸收迴路 (Snubber Network)**：在繼電器 NO 與 COM 接點兩端並聯一組 $0.1\,\mu\text{F}$ 耐壓 400V 金屬化聚丙烯薄膜電容與 $100\,\Omega$ 1W 電阻，能有效吸收接點斷開時的高壓火花，延長繼電器接點壽命（實際效果依負載特性而定）。
* **物理安全距離 (Creepage Distance)**：市電**絕對不可**接在麵包板上；在洞洞板／PCB 上，高壓交流走線與低壓 Arduino 訊號線應依安規保持足夠爬電距離（市電強化絕緣一般約 6~8 mm），並加裝外殼防觸電。
...
- **變更摘要**: 全面檢視概念與敘述正確性並修正：
  - 熱釋電元件輸出為皮安級電流（非微安級），需內建 JFET 放大；8~14 µm 屬長波紅外（非中紅外）
  - HC-SR501 感應距離 3~7 m、錐角約 110° 以內，工作電壓約 4.5~20V
  - 5V 繼電器線圈電流約 70~90 mA（非數百 mA）
  - 修正光耦隔離觀念：強電隔離由繼電器本身提供；光耦需移除 JD-VCC 跳線並獨立供電才真正隔離
  - 非阻塞比較表：迴圈含 delay(10)，非「小於 10 微秒」；delay() 期間中斷仍會執行
  - 接點熔焊主因為電容性負載湧浪電流；刪除無根據的「壽命延長 5~10 倍」；市電不可接麵包板
