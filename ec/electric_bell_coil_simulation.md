...
瞬間感應出高達數百伏特的反向高壓。此電壓超過空氣擊穿電壓（約 $3 \text{ kV/mm}$，當微小間隙時數十伏即擊穿），在接點處產生微小的藍白色火花（Spark / Arc）。
...
其中：
1. **電磁力**：
   $$F_{mag}(I, x) = \begin{cases} \frac{\mu_0 A N^2 I(t)^2}{2 (g_0 - x)^2} & (\text{若電路閉合}) \\ 0 & (\text{若電路斷開}) \end{cases}$$
2. **接觸螺釘恢復力**：
...
| **常見用途** | 早期電話、老式門鈴、玩具實驗 | 傳統市話振鈴（90V AC 20Hz）、學校鐘聲、消防警報 |

---

### 3.2 接觸點電弧抑制與電磁相容性 (EMC)
電鈴工作時，電弧不僅損耗觸點，還會向空間輻射高頻電磁脈衝（EMP），干擾附近的無線電廣播與微控制器（MCU）。現代工業若使用機電蜂鳴器，必須採取以下防護：
