...
> **最後更新時間**: 2026-09-17 14:36:27 (UTC+8)  
...
1. **結構組成**：由一對經過特殊磁化熱處理的**鐵鎳合金（Fe-Ni Alloy）彈性簧片**組成，密封在充有高純度惰性氣體（如氮氣、氦氣）或真空的小型微型玻璃管中。簧片接點部位通常鍍有耐磨金屬（如銠 Rhodium、釕 Ruthenium 或金 Gold）。
2. **作動機制**：
...
| **機械與電氣壽命** | 約 10 萬 ~ 100 萬次 | 可達 1 億次以上 |
...
* **繼電器線圈需求**：以常見的 5V 繼電器（如 OMRON KS2E-M-DC5 或松樂 SRD-05VDC-SL-C）為例，其線圈電阻約為 $70\Omega \sim 125\Omega$。依歐姆定律計算：
...
* **結果**：此反向高壓脈衝可瞬間達到 **數十伏特至上百伏特（30V ~ 100V+）**，高壓倒灌會直接擊穿單晶片內部的 ESD 保護二極體與邏輯閘，導致微控制器永久損毀。
...
1. **三極管 / MOSFET 擴流開關**：
   * Arduino 腳位僅需輸出微弱的 **1 mA ~ 2 mA** 電流至 NPN 三極管（如 2N2222、S8050）的基極，使三極管進入飽和導通狀態，由外部電源與集極承載 70mA 的線圈電流。
2. **飛輪二極體 (Flyback Diode)**：
   * 在繼電器線圈兩端**反向並聯**一顆 1N4007 或高速 1N4148 二極體。
   * 斷電瞬間，反向感應電勢會透過二極體順向導通，形成局部閉合回路洩放電感能量，將電壓箝位在 0.7V 左右，徹底消除高壓突波。
3. **光耦合隔離模組 (Optocoupler)**：
   * 工業級模組（如 PC817 繼電器模組）更進一步實現「光電隔離」，使 Arduino 控制端與繼電器負載側完全沒有實體電氣共地，達到頂級抗干擾能力。
