...
> **最後更新時間**: 2026-09-16 10:11:01 (UTC+8)  
> **研究角色**: 科研學習者 / 專題研究者 (去識別化)  
...
1. [單元一：AI 提示詞工程 (Prompt Engineering) 與學術小論文共創方法論深度解析](#單元一ai-提示詞工程-prompt-engineering-與學術小論文共創方法論深度解析)
   - [1.1 科研提示詞五大漸進心法與思維模型 (Progressive Research Prompting)](#11-科研提示詞五大漸進心法與思維模型-progressive-research-prompting)
   - [1.2 「五輪漸進 Prompt」vs「新手常見誤區」深度對照表 (Comparative Analysis)](#12-五輪漸進-promptvs新手常見誤區深度對照表-comparative-analysis)
   - [1.3 人機協同科研閉環工作流 (Human-in-the-Loop Co-Creation Workflow)](#13-人機協同科研閉環工作流-human-in-the-loop-co-creation-workflow)
   - [1.4 實戰案例：五輪真實對話推進機制與學術產出深度解構 (Case Study & Interaction Dynamics)](#14-實戰案例五輪真實對話推進機制與學術產出深度解構-case-study--interaction-dynamics)
   - [1.5 科研進階延伸 Prompt 範本庫 (Advanced Prompt Templates: 數值回歸、紅隊評審、實驗排錯)](#15-科研進階延伸-prompt-範本庫-advanced-prompt-templates)
...
與 AI 協同撰寫學術小論文時，提示詞的設計依循研究認知規律，透過 **「五大漸進心法」** 步步深入：
...
1. **角色賦能與發散探索 (Persona & Ideation)**：在探索初期，以自然語言描述現象並賦予 AI 具啟發性的導師身份（如科學老師），引導模型從形態、側風、含水率等多維度發散選題。
2. **結構搭棚與精準收斂 (Structural Scaffolding)**：選定核心問題後，**切忌直接要求生成全文**，應先使用「搭棚法」規定大綱章節（題目、摘要、動機、文獻、方法），並可註明「結果可省略」，讓 AI 專注於前期架構。
3. **聯網檢索與理論深化 (Literature Deepening & Tool Delegation)**：授權 AI 進行聯網檢索，主動引入國際期刊的物理機制（如熱帶種子沉降、雷諾數相圖、羽毛球自對準力學）。
4. **理論反推方法之逆向校準 (Reverse Theoretical Calibration)**：**高階提示詞的靈魂在於「逆向追問」**。引導 AI 用檢索到的理論文獻反過來修正實驗設計，使實驗工具（ImageJ, Tracker, Arduino）與理論精確咬合。
5. **長上下文全域聚合 (Long-Context Global Synthesis)**：在多輪探討成熟後，發出總結指令調用大模型的長上下文記憶，將前述所有面向無省略整合為正式專題計畫書。

---

### 1.2 「五輪漸進 Prompt」vs「新手常見誤區」深度對照表 (Comparative Analysis)

| 研究推進階段 | ❌ 新手常見低效 Prompt (單向/空洞/揠苗助長) | ✅ 研究者實戰真實 Prompt (自然/精準/步步推進) | AI 認知反應機制與學術產出差異 |
| :--- | :--- | :--- | :--- |
...
### 1.3 人機協同科研閉環工作流 (Human-in-the-Loop Co-Creation Workflow)

在實際科研過程中，人類研究員與 AI 應形成緊密的「人機迴路 (Human-in-the-Loop)」：由人類負責真實世界的現象觀察、物理直覺判定與實體實驗執行；AI 則負責理論文獻檢索整理、多維微分方程推導、數據處理腳本撰寫與反向紅隊審查，從而大幅提升研究效率與論證嚴謹度。
...
### 1.4 實戰案例：五輪真實對話推進機制與學術產出深度解構 (Case Study & Interaction Dynamics)

本份筆記的研究成果並非透過單一 Prompt 憑空產生，而是專題研究者（去識別化）於 2026-09-16 上午 8:43 至 9:19 期間，透過 **「五輪遞進式對話 (5-Stage Progressive Prompting)」** 逐步引導 AI 從模糊發想淬鍊為專業學術論文的經典案例。以下針對這五輪互動的「提問意圖、AI 反應機制與學術產出價值」進行深度解構：

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│              專題研究者 ➔ AI 五輪學術小論文互動推進全景圖                        │
└─────────────────────────────────────────────────────────────────────────────────┘
...
### 1.5 科研進階延伸 Prompt 範本庫 (Advanced Prompt Templates)

當專題小論文初稿生成後，研究者可利用以下進階提示詞範本，進一步驅動 AI 完成數值擬合代碼生成、評審紅隊審查與實驗異常排錯：

#### 📌 範本一：數值擬合與視覺化腳本生成 Prompt
```text
...
```
* **應用時機**：完成實驗數據採集後，要求 AI 撰寫自動化分析與高質量論文圖表繪製程式。

#### 📌 範本二：科展評審紅隊批判性審查 Prompt
```text
請切換角色為全國科展特優評審委員，對這份研究計畫書進行最嚴格的批判性審查，挑出實驗設計、偏心率標定或 2.5m 落體高度邊界上的潛在漏洞，並提出具體修正建議。
```
* **應用時機**：正式定稿前，模擬評審視角挑出實驗漏洞與邏輯斷層，進行反向加固。

#### 📌 範本三：硬體韌體與時序控制程式 Prompt
```text
請撰寫一份 Arduino Nano 控制程式，透過伺服馬達實現「按鍵觸發 -> LED倒數3秒 -> 瞬間釋放夾爪 -> 蜂鳴器提示」之零初速落體釋放時序控制。
```
* **應用時機**：自動化搭建落體實驗裝置，確保操作變因具備高重現性。
...
### 🔹 [2026-09-16 09:58:48] [專題研究者 (去識別化) ➔ Gemini / Antigravity] 原生五輪 Prompt 協同全歷程存檔
- **研究者 / 提問者**: 專題研究者 / 科研學習者 (去識別化)
- **模型/Agent**: Gemini 3.7 Flash / Antigravity
- **原生對話推進五輪歷程記錄**:
...
根據目前選定的研究問題，統整以上討論，重新撰寫一份完整的研究大綱,要包含之前討論的內容與面向 完整的研究內容
```
- **變更摘要**: 完整登錄專題研究者自上午 8:43 至 9:19 與 AI 協同推進小論文構思的五輪原始真實提示詞，確立從發想、鎖定題目、文獻深化、方法修正到全文輸出的科研迭代典範。

### 🔹 [2026-09-16 10:11:01] [Gemini 3.7 Flash / Antigravity] 結構精簡與去重複化維護
- **模型/Agent**: Gemini 3.7 Flash / Antigravity
- **Prompt 原文**:
  ```text
  檢查是否有重複內容
  go
  ```
- **變更摘要**: 執行全域冗餘檢查與結構精簡。重構單元一為五大小節（心法框架、對照矩陣、閉環工作流、五輪案例解構、進階延伸範本庫），徹底消除 5 輪基礎提示詞在四處重複列出的冗餘，移除重複心法總結，並同步校正目錄與 HTML 頁面。
