...
└─────────────────────────────────────────────────────────────────────────────────┘
  [第 1 輪 08:43] 角色賦能與發散發想 ──► 設定生物老師，探索毛色/個性/落體等多元題目
         │
  [第 2 輪 09:11] 聚焦決策與骨架搭棚 ──► 鎖定「問題四：釋放姿態與軌跡」，要求大綱骨架
         │
...
* **何宗穎 Prompt 原文**：
  ```text
  你現在是一個大學生物老師，我對生物中的貓的毛髮顏色與個性和性別有興趣，如果想做一個小型的研究，提供我五個可以參考的論文題目
  ```
* **互動機制剖析**：
  - **Prompt 策略**：採用了 `角色設定（大學生物老師） + 興趣主題 + 具體輸出數量（五個題目）` 的清晰結構。
  - **AI 反應機制**：AI 進入學術導師模式，給出了包含「遺傳學、行為學、落體翻轉反射（Righting Reflex）與種子空氣動力學」等跨領域候選題目清單。
  - **協同價值**：幫助研究者快速瀏覽多個研究視角，為後續聚焦奠定基礎。
...
```text
請根據以上所有討論與修改，輸出完整、無省略、具備正式學術專題研究報告規範的完整研究計畫書。
```
