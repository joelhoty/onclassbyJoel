...
在初始階段明確指定 AI 的學術引導角色（如中學/大學科學老師、生物力學研究員），設定回答風格具備啟發性與可行性。
* **初期範例**：「你是科學老師，我想進行關於小葉欖仁的種子落下的相關研究，提供五個可行的研究問題讓我參考。」

#### 2. 逆向校準提示詞 (Grounding & Alignment Prompting)
當 AI 在中期文獻探討中帶入高階理論（如流體阻力相圖、質心壓心力偶）後，研究者**不需自己重新撰寫複雜公式**，只需使用逆向提示詞引導 AI 自我對齊。
* **中期核心指令**：「根據這些文獻探討，我的研究目的跟研究方法可以怎麼修改？」

#### 3. 負向約束與防幻覺機制 (Negative Constraints & Grounding)
避免 AI 幻想昂貴、不切實際或實驗室無法取得的儀器（如百萬級高速風洞、粒子影像測速儀 PIV 等）。
* **實例指令**：「【負向約束】：所有實驗設計必須基於高中/大專可行的平民化開源工具鏈：雙線懸掛法、ImageJ 數位影像處理、Arduino 微夾爪電控釋放、以及 240 fps 手機慢動作搭配 Tracker 視訊分析軟體。」

#### 4. 全域知識聚合與 Schema 約束 (Global Synthesis & Schema Templating)
在對話最後階段，利用大模型的長上下文記憶，發出終極統整指令，強制要求依據標準學術論文格式產出完整內容。
* **後期收斂指令**：「根據目前選定的研究問題，統整以上討論，重新撰寫一份完整的研究大綱，要包含之前討論的內容與面向 完整的研究內容。」

---

### 1.2 「分階段提示詞演進」vs「單純新手低效指令」對照表

| 研究階段 | ❌ 新手常見低效 Prompt (單向且空洞) | ✅ 專家級階梯式真實 Prompt (漸進引導) | 提示詞設計邏輯與 AI 反應機制 |
| :--- | :--- | :--- | :--- |
| **初期：選題探索** | `我想做小葉欖仁種子掉下來的題目，幫我寫論文。` | `你是科學老師，我想進行關於小葉欖仁的種子落下的相關研究，提供五個可行的研究問題讓我參考。` | **啟發引導**：不堆砌看不懂的術語，讓 AI 從不同物理維度（偏心、風布、含水率、姿態）拋出候選方案。 |
| **中期：框架搭棚** | `好，就做第四個，直接寫內容。` | `我要選擇研究問題四：初始釋放姿態對動態穩定性與軌跡的影響。撰寫研究大綱，包含題目,摘要,動機,文獻探討,研究方法,研究結果（可省略）,結果與討論(可省略）。` | **結構約束**：先搭骨架（Scaffolding），避免 AI 一次性生成全文時結構散亂。 |
| **中期：文獻深化** | `文獻探討太短了，再多寫一點。` | `把文獻探討寫得更詳細更多面向，可以利用網路搜尋找相關的研究納入。` | **工具調用**：授權 AI 聯網檢索，引入熱帶種子沉降、雷諾數相圖與 Nature 前沿論文。 |
| **後期：方法校準** | `幫我想實驗步驟。` | `根據這些文獻探討，我的研究目的跟研究方法可以怎麼修改？` | **逆向校準**：讓文獻中的剛體氣動力學理論反過來指導實驗工具（ImageJ, Tracker, Arduino）升級。 |
| **後期：全文統整** | `把前面的合在一起。` | `根據目前選定的研究問題，統整以上討論，重新撰寫一份完整的研究大綱,要包含之前討論的內容與面向 完整的研究內容。` | **知識聚合**：最大化利用長上下文記憶，產出無省略、高密度的正式學術計畫書。 |
